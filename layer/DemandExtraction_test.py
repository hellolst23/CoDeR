...
class DemandExtraction(Module):
    def __init__(self, n_demand, n_categories, embedding_dim_c, hidden_size, drop=0.25, predict_catgy_fun = 'mlp',score_act_fun=None, demand_extract='mlp',
        ...
        self.embedding_dim = embedding_dim_c
        ...
        self.embedding_position = PositionalEmbedding(n_categories, embedding_dim_c)
    ...
    def forward(self, input, candidate_pool_category, session_last_catgy_index,mask_catgy):
        ...
        # max_session = np.arange(0, max_session_len)
        embedding_c = self.embedding_c(input)  # batch_size * max_session_len * embedding_dim_c
        embedding_p = self.embedding_position(input) # PositionalEmbedding
        embedding = embedding_c + embedding_p
        ...
        if self.demand_mask:
            ...
            # demand_score_candidate = torch.ones((batch_size, self.n_demand, n_items)).to(device=input.device) 
        ...
        elif self.demand_agg == "attention": 
